...
find $LOG_DIR -name "*.log" -print0 | while IFS= read -r -d '' arquivo; do 
    ...
    echo "Arquivo: $nome_arquivo" >>  "${ARQUIVO_DIR}/log_stas_$(date +%F).txt"
    echo "Número de linhas: $num_linhas" >> "${ARQUIVO_DIR}/log_stas_$(date +%F).txt"
    echo "Número de palavras: $num_palavras" >> "${ARQUIVO_DIR}/log_stas_$(date +%F).txt"
    echo "-----------------------------"  >> "${ARQUIVO_DIR}/log_stas_$(date +%F).txt"
